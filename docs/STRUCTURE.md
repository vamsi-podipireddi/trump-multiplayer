...
| `app/js/coach/worker.js` | Module-worker entry: `handleRequest` for hint/review/report requests, guarded so it also imports cleanly in Node |
...
| `app/js/ui/coach.js` | Hint affordance, table-read panel, post-deal review, match report card — the coach's DOM-writing half |
...
(optional D1 stats table), `migrations/` (SQL to bring an existing D1 database's schema up to date,
applied by hand via `wrangler d1 execute` — never run automatically), `wrangler.toml`, `package.json`.
...
| L5 | `ai/bid-search.js` | `bidValue aiBidDecisionSearch aiPickTrumpSearch aiPickPartnerSearch evaluateTrumps evaluateCalls` + its three play budgets, and `worldsFor`/`withTrump` so `scripts/bench-auction-search.js` cannot fork the formula |
...
| L8 | `index.js` | barrel — re-exports the public surface, 45 names |
