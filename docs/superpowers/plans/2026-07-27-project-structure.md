...
- **Imports inside a module file under `app/js/` use RELATIVE specifiers** (`./labels.js`, `../core/engine/index.js`). **Imports written inside an HTML file use ABSOLUTE `/js/...` paths.** The split is not arbitrary: a relative specifier resolves against the URL of the file containing it, so `./session.js` written in `app/index.html` resolves to `/session.js` and 404s, while the same text inside `app/js/main.js` resolves correctly. Until Task 17 moves the inline script into `main.js`, `app/index.html` legitimately holds absolute imports; afterwards its only absolute path is the `<script type="module" src="/js/main.js">` tag.
