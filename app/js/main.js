...
import { send, serverNow, setViewHandler, setEmoteHandler } from "./net.js";
...
/* Three seams survive even though render(), showEmote() and renderSettings()
   ...
   that can import every real function and hand it to the module that needs it. */
function boot() {
  setViewHandler(render);
  setEmoteHandler((seat, e) => showEmote(seat, e, posOfSeat));
  ...
  $("btn-help").onclick = () => showHelp(S.view);   // not the handler itself: a MouseEvent is a truthy `view`
  ...
  /* ---------- first paint ----------
     ...
     the deck's tables are `const`s in a module this one imports — calling into
     them any earlier throws on the temporal dead zone and takes the rest of the
     boot with it. */
  paintIcons(document);
  (function dressChrome() {
    ...
    for (const id of ["brand-colophon", "lobby-colophon"])
      $(id).innerHTML = `<i class="bar"></i><span>250 points · bid &amp; capture</span>`;
