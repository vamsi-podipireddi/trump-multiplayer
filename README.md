...
| **AI skill** | `easy` / `normal` / `hard`. Hard's card play is Perfect-Information Monte Carlo: it samples deals consistent with everything it has seen (voids, the called card) and rolls each candidate move out, under a fixed budget of simulated card plays. Hard also searches the auction — the bid, trump and partner call — instead of the linear hand-count `easy`/`normal` use, which is what gives the three tiers a real identity beyond card play. Changeable mid-match. |
