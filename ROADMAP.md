...
- [ ] PWA: `public/manifest.json`, `public/sw.js` (precache / + /solo.html, offline → solo), SW registration,
      `public/solo.html` = copy of root offline game; node server serves public/* correctly.
- [ ] A11y: cards focusable buttons with aria-labels ("play ace of spades"), Enter/Space to play, log as
      aria-live polite, 4-color deck toggle (♦ blue ♣ green) persisted in localStorage.
- [ ] Share: copy-link button + Web Share API in lobby; OpenGraph/meta tags.
- [ ] Commit M7.
